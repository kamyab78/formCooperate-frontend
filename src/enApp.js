...
function submit(){
  ...
  if(degree===null){
    toast.error('fill Scientific degree')
    return
  }
  if(workexp===null){
    toast.error('fill Work experience')
    return
  }
  if(supportdes===null){
    toast.error('fill How to support the project')
    return
  }
  if(expdes===null){
    toast.error('fill Resume')
    return
  }
  const body= {
  ...
  return (
    ...
    <Header/>
       ...
       <div style={{display:'flex',flexDirection:'row', justifyContent:'center',alignItems:'center',marginTop:'20px'}}>
          <input   type="radio" id="persian" name="fav_language" value="persian" onClick={()=>window.location.replace('/user?partnerid=1')}/>
